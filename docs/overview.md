...
- **Phase 3A-D Agent/MCP：Phase 3A/3B 部分实现。** V15-V17 已落地会话、分组、turn 和持久队列；V18 新增模型连接、价格版本、Usage、工具 Trace 和会话/turn 模型快照。设置页可安全配置 HTTPS 模型端点、API Key 和 Token 单价，首页可按会话切换模型并展示服务端持久化的 TTFT、逐轮/逐工具耗时、Token 和估算成本。自动评测、受控确认 UI 和 MCP Server 尚未完成。
