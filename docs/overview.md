...
> 状态日期：2026-09-23
> 当前主线：Phase 3B 增量实施中，真实模型已能调用七个只读工具以及新增工时、单笔收入/支出的 R2 prepare；commit 仅由站内确认触发，仍按“现有功能 Agent 化 → MCP → 文件/RAG”顺序推进
...
- **Phase 3A-D Agent/MCP：Phase 3A/3B 增量实施。** 会话、队列、SSE、模型连接、Usage 与 Trace 已落地；首页已支持新增工时和单笔收入/支出的白名单动态表单、服务端预览、站内确认和幂等 commit。补参后会更新历史 action 元数据，刷新可恢复最新终态；V19 增加固定价和 DeepSeek 峰谷计价。自动评测、修改删除、复杂账本写入和 MCP Server 尚未完成。
