...
> 版本：v1.8（2026-09-23）
> 状态：实施中（阶段 2 已完成真实 DeepSeek、R0/R1 Domain Tool、服务端会话恢复、基础 turn/SSE 和回复可观测元数据；持久化完整 trace、自动评测和受控写入仍按后续增量建设）
...
当前增量继续完善 Phase 3B 的会话组织和页面内队列交互，不开放写工具或 MCP 入口。
...
- 同一会话正在生成回复时允许继续提交消息，后续消息进入前端 FIFO 队列；队列显示等待数量和内容，可移除单条或清空，当前 turn 完成后按序执行。该队列目前是页面内临时状态，刷新不会恢复，服务端持久化队列属于后续稳定化范围。
...
- 页面内待发送队列支持拖拽排序；调整的是当前会话消息在全局 FIFO 中的相对顺序，不改变其他会话队列槽位。队列仍是临时状态，刷新不会恢复。
- 自动跟随改为严格底部判定：用户任何向上滚轮或实际向上滚动都会立即关闭跟随，只有真正滚动到 2px 内的底部或主动点击“滚动到底部并继续跟随”才重新开启，避免增量输出与用户上拉争抢滚动位置。

本次明确未实施：

- 持久化完整 Agent trace、动态表单和受控写入 UI；当前已能恢复会话和已完成消息，进行中的 turn 断流后可查询状态，但尚未建设独立 trace/usage 表和后台任务接管。
...
当前判定：本增量已完成阶段 2 的核心恢复链路，但仍未满足完整退出门禁。首页真实模型已能通过 SSE 增量输出、恢复服务端历史、展示 turn 与详细性能元数据，并在断流后对账；下一增量优先补自动中文评测、取消/重试体验和稳定性门禁，不启动 MCP。
...
5. 观察发送后立即出现“正在思考…”；Markdown、真实 SSE 增量、手动上滚立即停止吸附和恢复到底部按钮应正常。
6. 回复生成期间连续发送两条消息，确认后续消息进入可见队列，并可移除单条或清空；当前回复结束后剩余消息按顺序发送。
...
9. 回复生成期间再发送至少两条消息，在等待队列中拖拽改变顺序，确认后续实际回答按新顺序执行。
...
**当前状态：部分完成。** 首页已接入真实 DeepSeek，最小 `AgentOrchestrator` 能选择和串联当前用户可用的 R0/R1 Domain Tool；服务端会话、turn、基础 SSE、断流恢复、会话分组/置顶已完成，完整 trace、自动评测和服务端队列恢复尚未完成。
...
- [x] 增加页面内 FIFO 消息队列和队列管理；严格区分用户上滚与程序滚动，避免流式回复阻止用户浏览历史。
- [x] 服务端持久化会话分组与置顶；前端支持创建/改名/删除分组、折叠、菜单移动及桌面拖拽进出分组，删除分组不删除会话。
- [x] 页面内 FIFO 消息队列支持拖拽重排，并以实际执行顺序验证。
- [ ] 将进行中 turn 和待发送队列持久化到服务端，支持刷新、跨标签页和多实例恢复。
