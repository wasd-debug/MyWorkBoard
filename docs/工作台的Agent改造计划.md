...
> 版本：v2.0（2026-09-23）
> 状态：实施中（阶段 3 已开始：真实模型、R0/R1 查询、服务端会话/队列/SSE、模型连接与 Trace 已完成，新增工时和单笔收入/支出已进入受控写入）
...
当前增量完成 Phase 3B 首批受控写入：新增工时与单笔收入/支出使用 prepare、动态补参、站内确认和 commit；不开放修改、删除或 MCP 写入口。
...
- 首页助手消息现显示消息时间、工具调用明细、Token 用量和总耗时；用户与助手消息均可复制。工具与 Token 明细互斥展开，点击弹层以外区域会自动收起。元数据会随本地会话一同保存，刷新后仍可查看。
- 会话列表移除双击改名和行内删除入口，桌面端支持右键打开会话菜单，触屏与键盘可使用“更多”按钮；菜单提供重命名、归档和删除，删除要求二次确认，菜单支持外部点击、Esc、滚动关闭和方向键导航。
- 新增 V17 服务端队列结构：`agent_session.queue_revision`、`agent_turn.queue_position` 和 `retry_of_turn_id`。同一会话严格串行领取队首，等待项、执行中状态、排序版本和失败重试均由 MySQL 持有，刷新、跨标签页和应用重启后可恢复。
- 队列新增查询、入队、带 revision 重排、移除等待项、取消执行中 turn 和失败/取消重试接口；重排冲突会回读服务端顺序，同一原始 turn 最多生成一个 retry turn。
- 服务启动时把遗留 `RECEIVED / PLANNING` turn 重新放回 `QUEUED`，定时扫描未运行队列；后台执行重新加载用户及权限，不保存 JWT。同一会话完成、失败或取消后自动领取下一条。
- 前端首条消息继续使用 SSE；后续消息直接进入服务端队列并轮询状态/历史结果。执行中可停止、等待项可删除和重排、失败/取消回复可重试；队列轮询不会重新开启已被用户关闭的底部跟随。
- 新增 V16 `agent_session_group`、`agent_session.group_id` 和 `pinned_at`；分组、置顶状态按用户保存在服务端，删除分组只把会话移到“未分组”，不会删除会话。
...
- 模型工具白名单扩展到明确允许的 R2 `*.prepare`，但继续拒绝向模型暴露任何 `*.commit`。SSE 增加 `input.required` 和 `confirmation.required`，最终回复元数据保存 action 快照，刷新后可恢复卡片并回查服务端状态。
- 首页新增白名单动态表单、预览确认和结果卡片；补参生成新 action 并取消旧 action，拒绝不会写业务数据，确认流程固定为站内 `approve → commit`。工时和账本写入完成后分别刷新现有工时 store 和账本本地投影。
...
- 工时修改/删除、流水修改/删除、复杂账本类型、实体歧义选择和 revision 冲突专用卡片。
...
- Agent 专项 E2E 在桌面 Chromium、桌面 WebKit 与 375px 移动 Chromium 共 9/9 通过，覆盖 SSE 增量、服务端刷新恢复、详细 Token/耗时弹层及普通请求降级；WebKit 验证复制状态，Chromium 额外验证剪贴板内容。
- 2026-09-23 会话交互增量：`npm run build` 通过；桌面 Chromium 新增右键/更多菜单、重命名、归档、删除确认、消息排队和严格滚动跟随回归，聚焦新增用例 2/2 通过。
...
4. 询问“帮我记今天工时”，确认出现缺参表单；补充日期和开始时间后出现服务端预览，只有点击“确认并保存”才产生记录。
5. 询问“今天 9 点上班，20:30 下班，休息 60 分钟”，确认直接出现工时预览，拒绝时零写入，重复确认不能创建两条。
6. 询问“午饭 28 元”，确认缺少账本、账户或二级分类时显示受控表单；信息齐全后显示收入/支出、金额、账户、分类、日期和备注预览。
7. 观察发送后立即出现“正在思考…”；Markdown、真实 SSE 增量、手动上滚立即停止吸附和恢复到底部按钮应正常。
6. 回复生成期间连续发送两条消息，确认后续消息进入服务端队列，并可移除单条或清空；刷新页面后队列仍存在，当前回复结束后剩余消息按顺序发送。
...
10. 在“设置 → Agent 模型与成本”中新建 HTTPS 模型连接，配置模型名、API Key 与固定或 DeepSeek 峰谷 Token 单价并测试连接；系统环境模型也可单独保存成本配置。回到首页后通过输入框工具区切换模型，完成消息后展开“费用”查看总额和逐轮计价档位。不要在截图、日志或问题描述中粘贴完整 API Key。
...
- AI 工作台页面骨架；首页已从本地关键词回复切换到服务端 DeepSeek 聊天，并具备带服务端文本上下文的只读工具调用循环。发送后立即创建助手气泡，内容由服务端 SSE 真实增量填充，不再使用假打字机；助手内容支持经过标签白名单清理的 Markdown，用户向上滚动后停止吸附并提供恢复到底部按钮。服务端持久化 turn，刷新恢复已完成消息，断流可按 `turnId` 对账。
...
消息级导航入口只允许附着在助手消息上；用户消息只呈现用户输入、附件和消息元信息，不根据用户文本生成“进入账本”“进入工时”等入口。

输入框内的模型选择器使用紧凑工具按钮形态：默认只显示图标和短名称，完整模型名称保留在悬浮提示与下拉选项中，并限制控件尺寸，避免挤占文本输入空间。

助手消息从服务端历史恢复后按 `turnId` 补载 Trace，保证页面刷新、队列轮询和完成后的消息重载不会丢失费用、Token、模型耗时和峰谷档位；Trace 返回每轮实际使用的 `pricingTier`。

2026-09-23 本地修复增量补充：LLM 网关兼容 DeepSeek 将工具调用编码在正文中的 DSML 响应，并在流式输出阶段截断协议文本、恢复一个回复中的多次工具调用。自然语言已经匹配到账户、有效二级分类、金额和日期时直接生成确认预览；多笔记账生成多个独立 action，不再重复询问已识别字段。动态表单的 `entity-picker` 改为可输入筛选的组合框，账户和分类均支持名称子串过滤；待补充和待确认卡片都支持取消，取消后立即进入终态且不写业务数据。移动端顶部继续保留配色、同步状态、设置和账户入口，以紧凑图标呈现，不与当前模块的底部二级导航混用。
...
- [x] 提供 `assistant.delta`、`tool.started`、`tool.completed`、`turn.completed`、`turn.failed` 基础 SSE；建连前失败时降级至普通聊天接口，建连后中断按 `turnId` 查询最终状态。
- [x] 记录并展示真实总耗时、首字时延、每轮模型耗时/首字、每个工具耗时，以及输入、输出、缓存命中/未命中、思考与回复 Token。
- [x] 完善会话列表右键/更多操作菜单，接入服务端改名、归档和删除，并提供受控对话框与删除确认。
- [x] 增加服务端 FIFO 消息队列和队列管理；严格区分用户上滚与程序滚动，避免流式回复或队列轮询阻止用户浏览历史。
- [x] 服务端持久化会话分组与置顶；前端支持创建/改名/删除分组、折叠、菜单移动及桌面拖拽进出分组，删除分组不删除会话。
- [x] 服务端 FIFO 消息队列支持带 revision 的拖拽重排，并以刷新恢复后的顺序验证。
- [x] 将进行中 turn 和待发送队列持久化到服务端，支持刷新、跨标签页和应用重启恢复；支持运行中取消、等待项删除及失败/取消重试。
...
> 2026-09-23 前端导航补充：移动/平板底栏只承载当前模块二级菜单，不重复显示模块名，每项以图标为主要识别、短文字为辅助。系统按实际宽度动态决定直显数量，溢出入口进入“更多”；用户可按模块调整并持久化顺序，当前路由必须始终保持可达与高亮。此项属于既有工作台可用性改进，不改变 Agent、Domain Tool 或 MCP 分阶段边界。
