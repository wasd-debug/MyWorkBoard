...
| 数据库 | MySQL 8 + Flyway V1-V19；用户、工时、账本、同步、定时任务、审计、AI 会话/队列、模型连接、Usage、Trace 与峰谷计价 | 多用户和账本数据模型已落地；事件、任务、文件、RAG 和洞察读模型仍未落地 |
...
移动/平板（1024px 及以下）的底栏是当前模块的二级导航，而不是跨模块切换器。底栏不重复放置模块名，每项以图标为主要识别、短文字为辅助；菜单容量按底栏实际宽度动态计算，超出项进入“更多”。排序按模块保存在客户端，当前路由即使位于溢出项中也必须保持可达并呈现选中状态。跨模块返回统一由顶部“个人工作台”入口承担。

移动端应用顶栏仍需提供配色、同步状态、设置和当前账户入口，采用图标化紧凑布局；不得因隐藏桌面导航而一并移除这些全局操作，也不得把它们塞入模块二级底栏。Agent 的账户、分类等 `entity-picker` 使用可搜索组合框，展示名称但提交领域实体 ID；模型供应商若把工具调用以 DSML 等正文协议返回，网关必须在用户可见输出前恢复为统一 Tool Call 并过滤协议文本。
