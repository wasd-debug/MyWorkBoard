...
import com.salarytracker.ledger.LedgerModels.MonthlyAnalysis;
...
class LedgerReportAiServiceTest {
    ...
    void parsesDeepSeekMonthlyAnalysisJson() {
        MonthlyAnalysis result = service.parseAnalysis("""
                ```json
        ...
        assertThat(result.headline()).isEqualTo("支出有所收敛");
        assertThat(result.summary()).isEqualTo("本月结余改善，但居住类支出仍占比较高。");
        assertThat(result.suggestions()).hasSize(2);
        assertThat(result.risks()).containsExactly("收入来源较集中");
    }
