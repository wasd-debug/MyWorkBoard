...
class WorktimePaginationTest {
    ...
    void listRecordsAppliesTheRequestedOffset() throws Exception {
        ...
        verify(jdbcTemplate).queryForList(
                "SELECT id, date, TIME_FORMAT(start_time, '%H:%i') start_time, IFNULL(TIME_FORMAT(end_time, '%H:%i'), '') end_time, rest_min, overtime_min, real_hourly_wage, note, calc_version, timezone, revision FROM work_record WHERE user_id = ? AND deleted = FALSE ORDER BY date DESC LIMIT ? OFFSET ?",
                7L, 200, 200);
    }
}
