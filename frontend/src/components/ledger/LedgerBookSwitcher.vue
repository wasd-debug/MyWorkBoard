  ...
  <div class="ledger-book-switcher">
    ...
    <Sheet v-model:open="open" title="新建账本">
      <form class="book-form" @submit.prevent="create">
        ...
        <label>初始化方式<select v-model="form.mode"><option value="EMPTY">空白账本</option><option value="SYSTEM_TEMPLATE">系统基础模板</option><option value="COPY">复制已有账本</option></select></label>
...
const form = reactive({ name: '', currency: 'CNY', mode: 'EMPTY', sourceBookId: '' })
...
async function changeBook() {
  try {
    ...
    message.error(error?.response?.data?.detail || error?.message || '切换账本失败')
  }
}
async function create() {
  ...
  try {
    ...
    Object.assign(form, { name: '', currency: 'CNY', mode: 'EMPTY', sourceBookId: '' })
    open.value = false
    window.dispatchEvent(new CustomEvent('ledger-book-changed'))
  } catch (error) {
    message.error(error?.response?.data?.detail || error?.message || '新建账本失败')
  } finally {
    saving.value = false
  }
}
...
.ledger-book-switcher label,.book-form label { display:flex; min-width:0; flex-direction:column; gap:5px; color:var(--muted); font-size:12px }
