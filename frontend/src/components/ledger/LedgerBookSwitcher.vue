  ...
  <div class="ledger-book-switcher">
    <label>
      ...
      <select v-model="selected" :disabled="ledger.loading || !ledger.books.length" @change="changeBook">
        <option v-for="book in ledger.books" :key="book.id" :value="book.id">{{ book.name }}</option>
      </select>
    </label>
...
.ledger-book-switcher select { min-width:180px; height:32px; border:1px solid var(--line2); border-radius:3px; background:var(--paper); color:var(--ink); padding:0 8px }
