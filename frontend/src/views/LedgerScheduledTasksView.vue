...
import { apiListLedgerScheduledTasks } from '../../packages/api-client/src/index.js'
...
async function load() { loading.value=true; try { await ledger.init({waitForRemote:false}); tasks.value=ledger.online ? await apiListLedgerScheduledTasks(ledger.currentBookId) : [] } catch(error) { message.error(error.response?.data?.detail || error.message || '定时任务读取失败') } finally { loading.value=false } }
...
async function saveTask() { saving.value=true; try { if(form.taskType==='RECURRING_TRANSACTION'&&['EXPENSE','INCOME'].includes(payload.kind)){const category=ledger.categories.find(item=>String(item.id)===String(payload.categoryId||''));if(!category?.parentId||category.kind!==payload.kind)throw new Error('请选择对应流水类型的二级分类')} const request={...form,calendarRule:form.scheduleMode==='CALENDAR'?{...form.calendarRule}:{},payload:form.taskType==='RECURRING_TRANSACTION'?{...payload}:{}}; await ledger.createScheduledTask(request); formOpen.value=false; await load(); message.success('定时任务已创建') } catch(error) { message.error(error.response?.data?.detail||error.message||'保存失败') } finally { saving.value=false } }
async function toggleTask(task) { try { await ledger.updateScheduledTask(task,{enabled:!task.enabled}); await load() } catch(error) { message.error(error.response?.data?.detail||error.message||'更新失败') } }
async function runTask(task) { try { const result=await ledger.runScheduledTask(task); message.success(result.status==='APPLIED'?'已生成流水':result.status==='DUPLICATE'?'该日期已执行':'任务未执行'); await load() } catch(error) { message.error(error.response?.data?.detail||error.message||'执行失败') } }
async function removeTask(task) { if(!window.confirm(`删除“${task.name}”？`))return; try { await ledger.deleteScheduledTask(task); await load() } catch(error) { message.error(error.response?.data?.detail||error.message||'删除失败') } }
...
<style scoped>
.scheduled-page{max-width:1180px;margin:0 auto}.page-heading{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:22px}.eyebrow{color:var(--muted);font-size:12px;letter-spacing:.12em}.page-heading h1{margin:6px 0;font:700 30px Georgia,serif}.page-heading p:last-child{color:var(--muted);font-size:12px}.task-card{padding:0;overflow:hidden}.task-table{width:100%}.task-head,.task-row{display:grid;grid-template-columns:minmax(180px,2fr) 1.3fr 1.2fr 1.2fr 150px;gap:16px;align-items:center;padding:14px 18px}.task-head{background:var(--paper);color:var(--muted);font-size:12px}.task-row{border-top:1px solid var(--line);font-size:12px}.task-row small{display:block;color:var(--muted);font-size:12px;margin-top:4px}.failed{color:var(--down)}.actions{display:flex;gap:6px;justify-content:flex-end}.icon-button{width:30px;height:30px;border:1px solid var(--line2);border-radius:4px;background:var(--card);color:var(--ink2);cursor:pointer}.icon-button:hover{border-color:var(--accent);color:var(--accent)}.icon-button.danger:hover{color:var(--down)}.task-form{display:flex;flex-direction:column;gap:14px}.task-form label{display:flex;flex-direction:column;gap:6px;color:var(--muted);font-size:12px}.task-form input,.task-form select{height:34px;padding:0 9px;border:1px solid var(--line2);border-radius:4px;background:var(--card);color:var(--ink)}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.form-grid.single-rule{grid-template-columns:1fr}.schedule-mode{display:grid;grid-template-columns:1fr 1fr;gap:8px}.schedule-mode button{display:flex;min-height:58px;flex-direction:column;align-items:flex-start;justify-content:center;gap:4px;padding:9px 11px;border:1px solid var(--line2);border-radius:5px;background:var(--card);color:var(--ink2);text-align:left;cursor:pointer}.schedule-mode button.active{border-color:var(--accent);background:var(--accent-soft);color:var(--accent)}.schedule-mode small{color:var(--muted);font-size:12px}.hint{margin:0;padding:10px;background:var(--paper);color:var(--muted);font-size:12px;line-height:1.6}.task-form footer{display:flex;justify-content:flex-end;gap:8px;padding-top:6px}@media(max-width:700px){.task-head{display:none}.task-row{grid-template-columns:1fr auto;gap:8px}.task-row>span{font-size:12px}.task-row>span:nth-child(2),.task-row>span:nth-child(3){display:none}.page-heading{gap:10px}.form-grid,.schedule-mode{grid-template-columns:1fr}}
</style>
