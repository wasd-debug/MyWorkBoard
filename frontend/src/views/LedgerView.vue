<template>
  <section class="ledger-page">
    ...
    <div class="page-heading ledger-heading"><div><div class="label">PERSONAL FINANCE / LEDGER</div><h1>账本 <span class="heading-slash">// OVERVIEW</span></h1><p class="muted">记录每一笔流动，月底自动对账。</p></div><div class="ledger-actions"><LedgerActionIcon action="ai" label="自然语言记账" @click="aiOpen=true"/><LedgerActionIcon action="download" label="导出 CSV" @click="exportCsv"/><label class="ledger-import-icon" :class="{disabled:importState.active}" title="导入 CSV / Excel"><Upload aria-hidden="true"/><span class="sr-only">导入 CSV / Excel</span><input class="sr-only" type="file" accept=".csv,.xlsx,.xls,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" :disabled="importState.active" @change="importFile" /></label><LedgerActionIcon action="layout" label="编辑首页" @click="layoutOpen=true"/><LedgerActionIcon action="add" label="记一笔" @click="openCreate"/></div></div>
    ...
    <Card v-show="widgetVisible('summary','asset')" class="ledger-assets-card"><div><span class="summary-label">总资产</span><strong class="summary-value asset" :title="`¥${money(effectiveTotalAssets)}`">¥{{ money(effectiveTotalAssets) }}</strong></div><div class="ledger-assets-meta"><span>{{ accounts.length }} 个账户</span><button type="button" @click="openManager">查看账户 <b>→</b></button></div></Card>
    <div class="home-widget-grid">
      <div v-show="widgetVisible('content','period-summary')" class="widget-full period-summary-grid" :style="{order:widgetOrder('content','period-summary')}"><div v-for="item in periodSummaries" :key="item.label" class="period-summary-item"><div><strong>{{ item.label }}</strong><small>{{ item.range }}</small></div><div><span class="income">+¥{{ money(item.income) }}</span><span class="expense">−¥{{ money(item.expense) }}</span><b :class="item.net>=0?'income':'expense'">结余 ¥{{ money(item.net) }}</b></div></div></div>
...
import { apiLedgerAiPreview, apiListLedgerBudgets } from '../../packages/api-client/src/index.js'
...
async function loadData(){
  ...
  try{
    ...
    online.value=false
    message.error(error?.response?.data?.detail||error?.message||'账本刷新失败')
  }finally{
    syncing.value=false
    finishHomeLoading()
  }
...
function exportCsv(){const header='交易类型,日期,一级分类,二级分类,收入账户,金额,成员,商家,项目,备注';const rows=transactions.value.map(x=>{const c=categories.value.find(y=>String(y.id)===String(x.categoryId)),p=c&&categories.value.find(y=>String(y.id)===String(c.parentId));return[x.kind==='INCOME'?'收入':'支出',x.occurredOn,p?.name||x.categoryName||'',p?x.categoryName||'':'',x.accountName||'',x.amount,'',x.payee||'','',x.note||''].map(v=>`"${String(v).replaceAll('"','""')}"`).join(',')});const u=URL.createObjectURL(new Blob([`\ufeff${[header,...rows].join('\n')}`],{type:'text/csv;charset=utf-8'}));const a=document.createElement('a');a.href=u;a.download=`ledger-${selectedMonth.value}.csv`;a.click();URL.revokeObjectURL(u)}
 async function saveTransactionFixed(){
   if(form.kind==='TRANSFER' && !form.targetAccountId){message.error('请选择转入账户');return}
   try{
     ...
     message.success(ledgerStore.online?'交易已保存':'已离线保存，联网后自动同步')
   ...
   }catch(e){message.error(e.response?.data?.detail||e.message||'保存失败')}
   finally{pageLoading.value=false}
 }
 async function removeTransactionFixed(){if(!deleteTarget.value)return;deleting.value=true;try{await ledgerStore.deleteTransaction(deleteTarget.value);applyLedgerProjection();deleteTarget.value=null;message.success(ledgerStore.online?'流水已删除':'已离线删除，联网后自动同步');if(ledgerStore.online){await ledgerStore.syncNow();if(!ledgerStore.syncError)await ledgerStore.refreshAccounts();applyLedgerProjection()}}catch(e){message.error(e.response?.data?.detail||e.message||'删除失败')}finally{deleting.value=false}}
 async function saveCategoryFixed(){pageLoading.value=true;try{await ledgerStore.saveResource('category',{...categoryForm,parentId:categoryForm.parentId?String(categoryForm.parentId):null});Object.assign(categoryForm,{name:'',kind:'EXPENSE',parentId:''});applyLedgerProjection();message.success(ledgerStore.online?'分类已保存':'分类已离线保存')}catch(e){message.error(e.response?.data?.detail||e.message||'分类保存失败')}finally{pageLoading.value=false}}
 async function saveBudget(){
   const bookId=ledgerStore.currentBookId
   const monthKey=String(budgetForm.monthKey||'').trim()
   const amount=Number(budgetForm.amount)
   if(!bookId){message.error('请先选择账本');return}
   if(!/^\d{4}-(0[1-9]|1[0-2])$/.test(monthKey)){message.error('请选择有效月份');return}
   if(!Number.isFinite(amount)||amount<=0){message.error('预算金额必须大于 0');return}
   const label=budgetCategoryQuery.value.trim()
   const totalBudget=!label||label==='月度总预算'
   const matched=totalBudget?null:expenseCategories.value.find(c=>categoryPath(c)===label||c.name===label)
   if(!totalBudget&&!matched){message.error('请选择下拉列表中的一级或二级支出分类');return}
   pageLoading.value=true
   try{
     const payload={monthKey,budget:amount}
     ...
     applyStoreBudgets()
     message.success(ledgerStore.online?'预算已保存':'预算已离线保存')
   ...
   }catch(e){message.error(e.response?.data?.detail||'预算保存失败')}
   finally{pageLoading.value=false}
 }
 async function deleteBudgetFixed(budget){
   const bookId=ledgerStore.currentBookId
   if(!bookId||!budget?.id){message.error('预算信息不完整，请刷新后重试');return}
   pageLoading.value=true
   try{
     await ledgerStore.deleteResource('budget',budget)
     applyStoreBudgets()
     message.success(ledgerStore.online?'预算已删除':'预算已离线删除')
   ...
   }catch(e){message.error(e.response?.data?.detail||'预算删除失败')}
   finally{pageLoading.value=false}
 }
 async function saveAccountCommand(){
   ...
   try{
     ...
     applyLedgerProjection()
     message.success(ledgerStore.online?'账户已保存':'账户已离线保存')
   }catch(error){message.error(error.response?.data?.detail||error.message||'账户保存失败')}
   finally{pageLoading.value=false}
 }
 async function removeAccountCommand(account){
   try{await ledgerStore.deleteResource('account',account);applyLedgerProjection();message.success(ledgerStore.online?'账户已删除':'账户已离线删除')}
   catch(error){message.error(error.response?.data?.detail||error.message||'账户删除失败')}
 }
 async function removeCategoryCommand(category){
   try{await ledgerStore.deleteResource('category',category);applyLedgerProjection();message.success(ledgerStore.online?'分类已删除':'分类已离线删除')}
   catch(error){message.error(error.response?.data?.detail||error.message||'分类删除失败')}
 }
 async function aiPreviewRequestFixed(){
   ...
   }catch(e){message.error(e.response?.data?.detail||'DeepSeek 暂时不可用，请稍后重试')}
 ...
 async function confirmAiFixed(){pageLoading.value=true;try{if(!aiPreview.value?._draftId)throw new Error('请先识别交易');if(!aiCanConfirm.value)throw new Error('请先补全账户、成员和二级分类，并处理未匹配项');await ledgerStore.confirmAi(aiPreview.value._draftId,aiDrafts.value);aiOpen.value=false;aiPreview.value=null;applyLedgerProjection();message.success('AI 记账完成')}catch(e){message.error(e.response?.data?.detail||e.message||'记账失败')}finally{pageLoading.value=false}}
...
.ledger-heading{margin-bottom:18px}.heading-slash{color:var(--muted);font:600 12px ui-sans-serif;letter-spacing:.12em}.ledger-heading .muted{margin:8px 0 0}.ledger-actions{display:flex;gap:8px;flex-wrap:wrap}.import-label{cursor:pointer}.import-label.disabled{cursor:wait;opacity:.64}.ledger-status{display:flex;gap:8px;align-items:center;margin-bottom:14px;color:var(--up);font-size:12px}.ledger-status.offline{color:var(--warn)}.status-dot{width:7px;height:7px;border-radius:50%;background:var(--warn)}.status-dot.online{background:var(--up)}.import-progress{display:flex;flex-direction:column;gap:7px;margin:-2px 0 16px;padding:13px 15px;border:1px solid var(--line);border-radius:6px;background:var(--card)}.import-progress-head{display:flex;align-items:center;justify-content:space-between;gap:14px}.import-progress-head>span{display:flex;min-width:0;align-items:baseline;gap:9px}.import-progress-head b{font-size:12px}.import-progress-head small,.import-progress>small{overflow:hidden;color:var(--muted);font-size:12px;text-overflow:ellipsis;white-space:nowrap}.import-progress-head strong{font:650 13px Georgia,serif;font-variant-numeric:tabular-nums}.import-progress-track{height:6px;overflow:hidden;border-radius:6px;background:var(--line)}.import-progress-track i{display:block;height:100%;border-radius:inherit;background:var(--accent);transition:width .24s ease}.import-progress-track.processing i{background:linear-gradient(90deg,var(--accent),color-mix(in srgb,var(--accent) 42%,white),var(--accent));background-size:200% 100%;animation:import-progress-wave 1.4s linear infinite}@keyframes import-progress-wave{to{background-position:-200% 0}}.overview-toolbar{display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;color:var(--muted);font-size:12px}.overview-period-control{display:flex;align-items:center;gap:8px;min-width:0}.overview-range-mode{flex:0 0 auto}.period-picker{display:flex;border:1px solid var(--line2);border-radius:4px;overflow:hidden}.period-picker button{width:31px;border:0;background:var(--card);color:var(--ink2);font:22px Georgia}.period-picker input{width:112px;border:0;border-left:1px solid var(--line);border-right:1px solid var(--line);padding:0 7px;background:var(--card);color:var(--ink);font-size:12px}.home-custom-range{display:grid;grid-template-columns:minmax(124px,1fr) auto minmax(124px,1fr);align-items:center;gap:7px}.home-custom-range input{min-width:0;height:33px;padding:0 8px;border:1px solid var(--line2);border-radius:4px;background:var(--card);color:var(--ink);font-size:12px}.ledger-summary-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:12px}.ledger-summary-grid :deep(.card){min-height:126px}.summary-label,.summary-meta{color:var(--muted);font-size:12px}.summary-value{font:700 clamp(25px,3vw,34px)/1 Georgia,serif;margin:5px 0 10px}.income,.asset{color:var(--down)}.expense{color:var(--up)}.budget-over{color:var(--up)}.period-summary-grid{display:grid;grid-template-columns:repeat(3,1fr);border:1px solid var(--line);border-radius:4px;background:var(--card);margin-bottom:20px}.period-summary-item{padding:13px 16px;border-right:1px solid var(--line)}.period-summary-item:last-child{border:0}.period-summary-item>div{display:flex;justify-content:space-between;gap:8px}.period-summary-item small{color:var(--muted);font-size:12px}.period-summary-item>div+div{margin-top:9px;align-items:baseline;flex-wrap:wrap;font-size:12px}.period-summary-item b{margin-left:auto}.home-widget-grid{display:grid;grid-template-columns:minmax(0,1.65fr) minmax(250px,.8fr);gap:20px;margin-bottom:20px}.widget-full{grid-column:1 / -1}.widget-wide{grid-column:span 1}.widget-narrow{grid-column:span 1}.widget-half{grid-column:span 1}.records-accounts{display:flex;flex-direction:column;gap:20px;min-width:0}.chart-card{min-width:0}.head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.kicker{color:var(--muted);font-size:12px;letter-spacing:.16em;font-weight:600}.head h2{margin:4px 0 0;font:700 21px/1.2 Georgia,serif}.head-right{display:flex;gap:6px;align-items:center;color:var(--muted);font-size:12px}.chart{width:100%}.tall{height:260px}.medium{height:250px}.seg{display:flex;border:1px solid var(--line2);padding:2px;border-radius:3px}.seg button,.manager-tabs button{border:0;background:transparent;color:var(--muted);padding:5px 8px;font-size:12px}.seg button.active,.manager-tabs button.active{background:var(--accent-soft);color:var(--ink);font-weight:600}.donut-wrap{height:196px;position:relative}.donut{height:100%}.donut-label{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;pointer-events:none}.donut-label b{font:700 22px Georgia,serif}.donut-label span{font-size:12px;color:var(--muted);margin-top:5px}.budget-total,.budget-row{display:flex;justify-content:space-between;gap:8px}.budget-total{padding-top:11px;border-top:1px solid var(--line);font-size:12px;color:var(--muted)}.budget-list{display:flex;flex-direction:column;gap:14px}.budget-row{font-size:12px;color:var(--ink2);margin-bottom:6px}.budget-track{height:5px;background:var(--accent-soft);border-radius:4px;overflow:hidden}.budget-track i{display:block;height:100%;background:var(--accent)}.budget-track i.over{background:var(--up)}.weekdays,.calendar{display:grid;grid-template-columns:repeat(7,minmax(0,1fr))}.weekdays{border-bottom:1px solid var(--line)}.weekdays span{text-align:right;padding:7px 8px;color:var(--muted);font-size:12px}.day{min-height:76px;padding:8px;display:flex;flex-direction:column;align-items:flex-end;gap:3px;border-right:1px solid var(--line);border-bottom:1px solid var(--line);font-size:12px}.day:nth-child(7n){border-right:0}.day:nth-last-child(-n+7){border-bottom:0}.day b{color:var(--ink2)}.day.today b{width:20px;height:20px;border-radius:50%;display:grid;place-items:center;background:var(--accent);color:#fff}.transaction-table td b{display:block;color:var(--ink);font-size:13px}.transaction-table td small{display:block;color:var(--muted);font-size:12px;margin-top:2px}.home-resource{display:inline-flex;align-items:center;min-width:0;gap:7px}.home-resource>span{display:flex;min-width:0;flex-direction:column}.home-resource b,.home-resource small{max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.home-project{display:inline-flex!important;align-items:center;gap:4px}.home-project i{flex:0 0 6px;width:6px;height:6px;border:0;border-radius:50%}.amount{text-align:right!important;white-space:nowrap}.row-action,.text-button{border:0;background:transparent;color:var(--accent);padding:5px;cursor:pointer;font-size:12px}.row-action{color:var(--muted)}.row-action.danger:hover{color:var(--up)}.list-footer{display:flex;justify-content:space-between;margin-top:14px;color:var(--muted);font-size:12px}.ledger-empty{min-height:160px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;color:var(--muted)}.account{display:flex;align-items:center;gap:10px;width:100%;padding:11px 0;border:0;border-bottom:1px solid var(--line);background:transparent;text-align:left;color:var(--ink)}.account i{width:28px;height:28px;border:1px solid var(--line2);border-radius:50%;display:grid;place-items:center;color:var(--accent);font-style:normal}.account span,.manager-row span{display:flex;flex-direction:column;flex:1}.account small,.manager-row small{color:var(--muted);font-size:12px}.account strong{font-size:12px;color:var(--ink2)}.account-add{width:100%;margin-top:12px}.manager-row{display:flex;justify-content:space-between;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--line);font-size:12px}.side-empty,.ai-intro{color:var(--muted);font-size:12px}.form{display:flex;flex-direction:column;gap:12px}.form label{display:flex;flex-direction:column;gap:5px;color:var(--ink2);font-size:12px;font-weight:600}.form-kind{display:grid;grid-template-columns:1fr 1fr;padding:3px;border:1px solid var(--line);background:var(--paper)}.form-kind button{border:0;background:transparent;padding:9px;color:var(--muted)}.form-kind button.active{background:var(--card);color:var(--ink);box-shadow:0 1px 3px #0002}.form-footer{display:flex;justify-content:flex-end;gap:8px}.manager-tabs{display:flex;gap:4px;border-bottom:1px solid var(--line);padding-bottom:8px;margin-bottom:14px}.ai-button{width:100%;margin-top:10px}.ai-preview{margin-top:20px;border-top:1px solid var(--line);padding-top:16px}.ai-preview>b{font:700 30px Georgia,serif}.widget-settings{display:flex;flex-direction:column;gap:4px;margin-bottom:18px}.widget-settings-title{font-size:12px;color:var(--muted);letter-spacing:.08em;margin-bottom:4px}.widget-setting-row{display:flex;align-items:center;gap:8px;border-bottom:1px solid var(--line);padding:8px 0;font-size:12px}.widget-setting-row>span{flex:1}.widget-visibility{border:1px solid var(--line2);background:transparent;color:var(--muted);padding:4px 8px;cursor:pointer;font-size:12px}.widget-visibility.on{color:var(--accent);border-color:var(--accent)}
@media(max-width:1023px){.home-widget-grid{grid-template-columns:minmax(0,1fr) minmax(0,1fr)}}@media(max-width:767px){.ledger-actions{width:100%}.ledger-actions .ui-button,.ledger-actions .import-label{flex:1 1 auto}.overview-toolbar{align-items:flex-start;flex-direction:column}.overview-period-control{width:100%;align-items:stretch;flex-direction:column}.home-custom-range{grid-template-columns:minmax(0,1fr);width:100%}.home-custom-range span{display:none}.ledger-summary-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.ledger-summary-grid :deep(.card){min-height:112px;padding:15px}.summary-value{font-size:25px}.home-widget-grid{grid-template-columns:minmax(0,1fr)}.widget-full,.widget-wide,.widget-narrow,.widget-half{grid-column:1}.period-summary-grid{grid-template-columns:minmax(0,1fr)}.period-summary-item{border-right:0;border-bottom:1px solid var(--line)}.period-summary-item:last-child{border:0}.home-widget-grid{gap:12px;margin-bottom:12px}.day{min-height:57px;padding:5px 4px}.chart.tall,.chart.medium{height:220px}.head{flex-direction:column}.head-right{align-self:stretch;flex-wrap:wrap}.transaction-table{margin:0}.transactions :deep(.ui-table){width:100%;min-width:0;table-layout:fixed}.transactions :deep(th:first-child),.transactions :deep(td:first-child){width:84px;overflow:hidden;white-space:nowrap}.transactions :deep(th:nth-child(2)),.transactions :deep(td:nth-child(2)){width:auto;min-width:0;overflow:hidden}.transactions :deep(th:nth-child(3)),.transactions :deep(td:nth-child(3)),.transactions :deep(th:nth-child(4)),.transactions :deep(td:nth-child(4)){display:none}.transactions :deep(th:nth-child(5)),.transactions :deep(td:nth-child(5)){width:82px}.transactions :deep(th:last-child),.transactions :deep(td:last-child){width:58px}.transactions :deep(th),.transactions :deep(td){box-sizing:border-box;padding-left:4px;padding-right:4px}.transactions .row-action{padding:3px 1px;font-size:12px}.transaction-description{max-width:none;padding-left:8px!important}}
...
.ledger-import-icon:focus-within{outline:2px solid var(--accent);outline-offset:2px;box-shadow:0 0 0 4px color-mix(in srgb,var(--accent) 20%,transparent)}
...
.ai-eyebrow{display:block;color:var(--accent);font-size:12px;font-weight:700;letter-spacing:.16em}
.ai-panel-intro h3{margin:4px 0 3px;color:var(--ink);font:700 20px/1.2 Georgia,serif}
.ai-panel-intro p{margin:0;color:var(--muted);font-size:12px;line-height:1.6}
.ai-status{display:inline-flex;align-items:center;gap:6px;flex:0 0 auto;color:var(--muted);font-size:12px;white-space:nowrap}
...
.ai-panel-actions{display:flex;align-items:center;justify-content:space-between;gap:12px;color:var(--muted);font-size:12px}
...
.ai-preview-head>div{display:flex;align-items:baseline;gap:8px}.ai-preview-head b{color:var(--ink);font-size:13px}.ai-preview-head span{color:var(--muted);font-size:12px}
...
.ai-draft-head{display:flex;align-items:center;gap:8px;min-width:0}.ai-draft-index{color:var(--muted);font:600 10px Georgia,serif}.ai-kind{padding:2px 6px;border-radius:3px;background:var(--accent-soft);color:var(--accent);font-size:12px}.ai-draft-head>b{margin-left:auto;font:700 16px Georgia,serif}.ai-draft-head>b.income{color:var(--down)}.ai-draft-head>b.expense{color:var(--up)}.ai-draft-head>b.transfer{color:var(--ink2)}
.ai-draft-state{color:var(--down);font-size:12px}.ai-draft-state.invalid{color:var(--warn)}
.ai-draft-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px 10px;margin-top:10px}.ai-draft-fields label{display:flex;min-width:0;flex-direction:column;gap:4px;color:var(--ink2);font-size:12px;font-weight:600}.ai-draft-fields .ui-input,.ai-draft-fields select{height:32px;padding:0 8px;font-size:12px}.ai-note-field{grid-column:1/-1}
.ai-warning{display:flex;align-items:flex-start;gap:5px;margin:9px 0 0;color:var(--warn);font-size:12px;line-height:1.5}.ai-warning span{display:grid;place-items:center;flex:0 0 14px;width:14px;height:14px;border-radius:50%;background:var(--warn);color:#fff;font-size:12px;font-weight:700}
.ai-preview-footer{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:12px;padding-top:12px;border-top:1px solid var(--line);color:var(--warn);font-size:12px}.ai-preview-footer .ledger-action-icon{width:auto;padding:0 12px}
