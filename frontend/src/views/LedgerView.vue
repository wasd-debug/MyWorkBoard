<template>
  <section class="ledger-page">
    ...
    <Sheet v-model:open="aiOpen" title="自然语言记账"><p class="ai-intro">说说刚刚发生的交易，助手会整理成可确认的记录。</p><textarea v-model="aiText" class="ui-textarea" placeholder="例如：今天午饭花了 32 元"/><LedgerActionIcon class="ai-button" action="ai" :label="aiLoading?'识别中':'识别交易'" :disabled="aiLoading" @click="aiPreviewRequest"/><div v-if="aiPreview" class="ai-preview"><div v-for="(draft,index) in aiDrafts" :key="`${draft.occurredOn||'draft'}-${index}`" class="ai-draft"><div class="ai-draft-amount"><b :class="draft.kind==='INCOME'||draft.kind==='BORROW_IN'||draft.kind==='COLLECT_DEBT'?'income':'expense'">{{draft.kind==='INCOME'||draft.kind==='BORROW_IN'||draft.kind==='COLLECT_DEBT'?'+':'−'}}¥{{money(draft.amount)}}</b><span>{{draft.occurredOn}}</span></div><p>{{draft.note||'未填写备注'}}</p><div class="ai-resource-fields"><label>账户<select :value="draft.accountId||''" @change="updateAiDraft(index,{accountId:$event.target.value})"><option value="">请选择账户</option><option v-for="account in accounts" :key="account.id" :value="String(account.id)">{{account.name}}</option></select></label><label v-if="draft.kind==='TRANSFER'">转入账户<select :value="draft.targetAccountId||''" @change="updateAiDraft(index,{targetAccountId:$event.target.value})"><option value="">请选择账户</option><option v-for="account in accounts" :key="account.id" :value="String(account.id)">{{account.name}}</option></select></label></div><div v-if="requiresAiCategory(draft.kind)" class="ai-category-fields"><label>一级分类<select :value="draft.parentCategoryId||parentCategoryId(draft)" @change="updateAiCategory(index,{parentCategoryId:$event.target.value})"><option value="">请选择一级分类</option><option v-for="parent in aiCategoryParents(draft.kind)" :key="parent.id" :value="String(parent.id)">{{parent.name}}</option></select></label><label>二级分类<select :value="draft.categoryId||''" @change="updateAiCategory(index,{categoryId:$event.target.value})"><option value="">请选择二级分类</option><option v-for="category in aiCategoryChildren(draft)" :key="category.id" :value="String(category.id)">{{category.name}}</option></select></label></div><p v-if="draft.warnings?.length" class="ai-warning">{{draft.warnings.join('；')}}</p></div><LedgerActionIcon action="confirm" label="确认记账" :disabled="!aiCanConfirm||pageLoading" @click="confirmAi"/></div></Sheet>
...
const aiCanConfirm=computed(()=>aiDrafts.value.length>0&&aiDrafts.value.every(draft=>!draft.warnings?.length&&(!requiresAiCategory(draft.kind)||Boolean(draft.categoryId))))
...
async function draw(){
  ...
  const spent=+budgetSummary.value.spent||0,budget=+budgetSummary.value.budget||0
...
async function aiPreviewRequest(){if(!aiText.value.trim())return;aiLoading.value=true;try{aiPreview.value=await apiLedgerAiPreview(aiText.value)}catch(e){ElMessage.error('暂时无法识别')}finally{aiLoading.value=false}} async function confirmAi(){try{await apiLedgerAiConfirm(aiPreview.value);aiOpen.value=false;await loadData();ElMessage.success('AI 记账完成')}catch(e){ElMessage.error('记账失败')}}
 ...
 async function saveBudgetFixed(){pageLoading.value=true;try{await apiCreateLedgerBudget({monthKey:budgetForm.monthKey,categoryId:String(budgetForm.categoryId),amount:+budgetForm.amount});await loadData();ElMessage.success('预算已保存')}catch(e){ElMessage.error(e.response?.data?.detail||'预算保存失败')}finally{pageLoading.value=false}}
 async function aiPreviewRequestFixed(){if(!aiText.value.trim())return;aiLoading.value=true;try{const result=await apiLedgerAiPreview(aiText.value);const drafts=(result.drafts||[]).map(draft=>{const child=categories.value.find(item=>String(item.id)===String(draft.categoryId||''));return {...draft,parentCategoryId:draft.parentCategoryId||child?.parentId||''}});const draft=drafts[0];if(draft)aiPreview.value={...draft,_draftId:result.draftId,_allDrafts:drafts}}catch(e){ElMessage.error(e.response?.data?.detail||'暂时无法识别')}finally{aiLoading.value=false}}
 function updateAiDraft(index,patch){
   ...
   aiPreview.value={...draft,_draftId:aiPreview.value._draftId,_allDrafts:drafts}
 }
