...
- **Phase 3A-D Agent/MCP：Phase 3A/3B 入口部分实现。** 已建立 AI 物理模块、七个内部 R1 查询工具、功能覆盖与中文评测集、action JDBC 持久化、首个工时 prepare/commit 工具和最小受控 REST 入口；首页已接入真实 DeepSeek 聊天、即时思考气泡、安全 Markdown、假打字机和可恢复的底部跟随，但模型会话、工具调用循环、SSE 和 MCP 尚未实现。
